 ...
 * Phase 1: Dry Run V2 - With Enhanced Fixes
 * 
 * Fixes Applied:
 * 1. Enhanced regime detection (ADX + volatility + trend)
 * 2. Dynamic consensus thresholds (40% for ranging)
 * 3. Intelligent position management with wider SL in ranging
 * 4. Regime-appropriate strategy selection
 * 5. Trailing stops and profit booking
 */

import { BacktestEngineV2, BacktestConfig, OHLCV, BacktestResult } from './BacktestEngineV2';
...
/**
 * Fetch data using native https module
 */
function httpsGet(url: string): Promise<string> {
  return new Promise((resolve, reject) => {
    const req = https.get(url, {
      headers: {
        'User-Agent': 'SEER-Backtest/2.0',
...
async function fetchHistoricalData(
  ...
  console.log(`[DataFetch] Fetching ${symbol}: ${startDate.toISOString().split('T')[0]} to ${endDate.toISOString().split('T')[0]}`);
  ...
  console.log(`[DataFetch] ${symbol}: ${uniqueCandles.length} candles loaded`);
...
function generateReport(result: BacktestResult): string {
  const { config, metrics, trades, validation, strategyPerformance, regimeStats, verdict, verdictReason } = result;
  
  let report = `
================================================================================
                    SEER A++ INSTITUTIONAL GRADE BACKTEST
                      PHASE 1 V2: ENHANCED REAL DATA TEST
================================================================================

ENHANCEMENTS APPLIED
--------------------------------------------------------------------------------
✅ Enhanced regime detection (ADX + volatility + trend strength)
✅ Dynamic consensus thresholds (40% for ranging, 25% for trending)
✅ Intelligent position management with regime-adaptive SL/TP
✅ Wider stops in ranging markets (2.5x ATR vs 2.0x)
✅ Regime-appropriate strategy selection
✅ Trailing stops with profit booking
...
`;
  ...
  for (const [regime, stats] of regimeStats) {
    ...
    report += `${regime.padEnd(12)} ${stats.trades.toString().padStart(6)} ${winRate.toFixed(1).padStart(6)}% ${pnlStr.padStart(12)}\n`;
  }
...
`;
  ...
  report += 'Time             Symbol    Side   Entry      Exit       P&L      Regime    Exit Reason\n';
  report += '─'.repeat(95) + '\n';
  ...
  for (const trade of lastTrades) {
    ...
    report += `${time.padEnd(16)} ${trade.symbol.padEnd(9)} ${trade.side.padEnd(6)} $${trade.entryPrice.toFixed(2).padStart(9)} $${trade.exitPrice.toFixed(2).padStart(9)} ${pnlStr.padStart(10)} ${trade.regime.padEnd(9)} ${trade.exitReason}\n`;
  }
...
--------------------------------------------------------------------------------
✅ Enhanced Regime Detection: ADX + Volatility + Trend Strength
✅ Dynamic Thresholds: 40% ranging, 35% volatile, 25% trending
✅ Position Management Agent: Adaptive SL/TP based on regime
✅ Trailing Stops: Activated after 1% profit
✅ Regime-Specific Strategies: Mean reversion for ranging, trend for trending
...
PHASE 1 V2 CONCLUSION:
${validation.isValid ? '✅ System validated - Ready for Phase 2 (2-year backtest)' : '❌ Issues detected - Fix before proceeding'}
 ...
 * Main execution
 */
async function runPhase1V2(): Promise<BacktestResult> {
  console.log('='.repeat(80));
  console.log('SEER A++ INSTITUTIONAL GRADE BACKTEST - PHASE 1 V2: ENHANCED');
  console.log('='.repeat(80));
  console.log('\nEnhancements:');
  console.log('  • Enhanced regime detection (ADX + volatility + trend)');
  console.log('  • Dynamic consensus thresholds (40% ranging, 25% trending)');
  console.log('  • Intelligent position management with wider SL in ranging');
  console.log('  • Regime-appropriate strategy selection');
  console.log('  • Trailing stops and profit booking');
  ...
  console.log(`\n[Phase1V2] Configuration:`);
  console.log(`  Period: ${startDate.toISOString().split('T')[0]} to ${endDate.toISOString().split('T')[0]}`);
  console.log(`  Symbols: ${config.symbols.join(', ')}`);
  console.log(`  Initial Capital: $${config.initialCapital.toLocaleString()}`);
  
  // Fetch real historical data
  console.log(`\n[Phase1V2] Fetching real historical data from Coinbase...`);
  const priceData = new Map<string, OHLCV[]>();
  
  for (const symbol of config.symbols) {
    const candles = await fetchHistoricalData(symbol, startDate, endDate, 300);
    ...
    console.log(`[Phase1V2] ${symbol}: $${Math.min(...prices).toFixed(2)} - $${Math.max(...prices).toFixed(2)}`);
  }
  
  // Run enhanced backtest
  console.log(`\n[Phase1V2] Running enhanced backtest...`);
  const engine = new BacktestEngineV2(config);
  ...
  engine.on('trade_opened', (trade: any) => {
    ...
    if (tradeCount <= 5 || tradeCount % 10 === 0) {
      ...
      console.log(`[Trade #${tradeCount}] ${time} OPEN ${trade.symbol} ${trade.side.toUpperCase()} @ $${trade.entryPrice.toFixed(2)} | ${trade.regime} | ${trade.strategy}`);
    }
  });
  ...
  engine.on('trade_closed', (trade: any) => {
    const pnlStr = trade.pnl >= 0 ? `+$${trade.pnl.toFixed(2)}` : `-$${Math.abs(trade.pnl).toFixed(2)}`;
    if (tradeCount <= 5 || tradeCount % 10 === 0) {
      console.log(`[Trade #${tradeCount}] CLOSE @ $${trade.exitPrice.toFixed(2)} | P&L: ${pnlStr} | ${trade.exitReason}`);
    }
  });
  
  const result = await engine.run(priceData);
  
  console.log(`\n[Phase1V2] Backtest completed. Total trades: ${result.trades.length}`);
  ...
  const reportPath = `/home/ubuntu/seer/docs/PHASE1_V2_REPORT_${timestamp}.txt`;
  fs.writeFileSync(reportPath, report);
  console.log(`\n[Phase1V2] Report saved to: ${reportPath}`);
  
  const jsonPath = `/home/ubuntu/seer/docs/PHASE1_V2_RESULTS_${timestamp}.json`;
  fs.writeFileSync(jsonPath, JSON.stringify({
    ...
    regimeStats: Object.fromEntries(result.regimeStats),
  ...
  console.log(`[Phase1V2] JSON saved to: ${jsonPath}`);
...
runPhase1V2()
  .then(result => {
    console.log(`\n[Phase1V2] Completed with verdict: ${result.verdict}`);
    process.exit(result.validation.isValid ? 0 : 1);
  })
  .catch(error => {
    console.error('[Phase1V2] Fatal error:', error);
    process.exit(1);
  });
