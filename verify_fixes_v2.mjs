...
`);
if (recentConsensus[0].count > 0) {
  console.log(`  ✅ Records in last hour: ${recentConsensus[0].count}`);
} else {
  console.log(`  ⚠️ No records in last hour (engine may need to generate signals first)`);
}

// 3. Check engine state for user 272657
console.log("\n3. ENGINE STATE (User 272657):");
const [engineState] = await conn.query(`SELECT * FROM engineState WHERE userId = 272657 ORDER BY id DESC LIMIT 1`);
...
const [tradingMode] = await conn.query(`SELECT * FROM tradingModeConfig WHERE userId = 272657`);
...
`);
if (recentSignals[0].count > 0) {
  console.log(`  ✅ Signals in last hour: ${recentSignals[0].count}`);
} else {
  console.log(`  ⚠️ No signals in last hour (engine may need to start)`);
}
...
if (decisionsWithPos[0].count > 0) {
  console.log(`  ✅ Trade execution logging is working!`);
}

// 7. Check recent positions
console.log("\n7. RECENT POSITIONS (last 24 hours):");
const [recentPositions] = await conn.query(`
  SELECT COUNT(*) as count, MAX(createdAt) as latest 
  FROM paperPositions 
  WHERE createdAt > DATE_SUB(NOW(), INTERVAL 24 HOUR)
`);
console.log(`  Positions in last 24h: ${recentPositions[0].count}`);
