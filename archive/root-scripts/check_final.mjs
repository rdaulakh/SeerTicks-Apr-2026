...
console.log('\n=== paperTrades schema ===');
const [cols] = await connection.query('DESCRIBE paperTrades');
console.log('Columns:', cols.map(c => c.Field).join(', '));

console.log('\n=== Recent Paper Trades ===');
const [trades] = await connection.query(
  "SELECT * FROM paperTrades WHERE userId = 272657 ORDER BY id DESC LIMIT 5"
);
console.log('Recent trades:', trades.length);
if (trades.length > 0) console.log(JSON.stringify(trades[0], null, 2));
